-- =================================================================
-- == SCRIPT DE INICIALIZAÇÃO COMPLETO PARA O BANCO DE DADOS CHECKNF-GDM ==
-- =================================================================
-- Este script irá apagar as tabelas existentes para garantir uma
-- instalação limpa e correta.
-- FAÇA UM BACKUP SE VOCÊ TIVER DADOS IMPORTANTES.
-- =================================================================
...
/*
  ...
  ## Query Description: This section will drop all existing tables, functions, and policies related to the application. This is to ensure a clean and correct re-initialization. This is a DESTRUCTIVE operation and will delete all existing data in these tables.
...
/*
  ...
  ## Query Description: Creates the table to store client information. This table will be populated with the initial client list.
...
/*
  # [Operation Name]
  Create 'fretistas' table

  ## Query Description: Creates the table to store freight driver information. This table will be populated with the initial driver list.
...
COMMENT ON TABLE public.fretistas IS 'Armazena informações dos fretistas.';

/*
  # [Operation Name]
  Create 'usuarios' table

  ## Query Description: Creates the table for user profiles, linked to Supabase Auth. It includes the 'fretista_placa' column to associate users with drivers. This is the corrected version.
...
CREATE TABLE public.usuarios (
  ...
  fretista_placa TEXT REFERENCES public.fretistas(placa) ON DELETE SET NULL, -- CORREÇÃO APLICADA AQUI
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
COMMENT ON TABLE public.usuarios IS 'Perfis de usuário, estendendo auth.users.';

/*
  # [Operation Name]
  Create 'notas_fiscais' table

  ## Query Description: Creates the main table for storing invoice data.
...
CREATE TABLE public.notas_fiscais (
  ...
  situacao TEXT DEFAULT 'Dentro do Prazo' CHECK (situacao IN ('Dentro do Prazo', 'Vencimento Próximo')),
  status TEXT DEFAULT 'Pendente' CHECK (status IN ('Pendente', 'Entregue', 'Cancelada', 'Devolvida', 'Reenviada', 'Paga')),
  observacao TEXT,
  arquivo_url TEXT,
  usuario_registro UUID REFERENCES auth.users(id),
  data_registro DATE DEFAULT CURRENT_DATE,
  hora_registro TIME DEFAULT CURRENT_TIME,
  usuario_edicao UUID REFERENCES auth.users(id),
  data_edicao DATE,
  hora_edicao TIME,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
...
/*
  ...
  ## Query Description: Creates a table to log user actions within the application.
...
COMMENT ON TABLE public.historico_acessos IS 'Registra as ações e acessos dos usuários.';
...
/*
  ...
  ## Query Description: This function automatically creates a profile in the 'public.usuarios' table whenever a new user is created in 'auth.users', keeping them in sync.
...
COMMENT ON TRIGGER on_auth_user_created ON auth.users IS 'Cria um perfil em public.usuarios para cada novo usuário em auth.users.';
...
/*
  # [Operation Name]
  Populate 'fretistas' table

  ## Query Description: Inserts the initial list of drivers into the 'fretistas' table.

  ## Metadata:
  - Schema-Category: "Data"
  - Impact-Level: "Low"
  - Requires-Backup: false
  - Reversible: true (by deleting the data)
*/
...
/*
  ...
  ## Query Description: Inserts the initial list of clients into the 'clientes' table.
...
/*
  ...
  ## Query Description: Enables Row Level Security on all tables and creates policies to control data access based on user roles ('tipo' in the 'usuarios' table). This is critical for application security.
...
CREATE POLICY "Admins podem gerenciar todos os usuários." ON public.usuarios FOR ALL USING (((SELECT tipo FROM public.usuarios WHERE (usuarios.id = auth.uid())) = 'administrador'::text));
CREATE POLICY "Usuários podem ver e atualizar seus próprios perfis." ON public.usuarios FOR ALL USING ((auth.uid() = id));

-- Policies for 'clientes'
CREATE POLICY "Admins podem gerenciar todos os clientes." ON public.clientes FOR ALL USING (((SELECT tipo FROM public.usuarios WHERE (usuarios.id = auth.uid())) = 'administrador'::text));
CREATE POLICY "Usuários autenticados podem ler clientes." ON public.clientes FOR SELECT USING ((auth.role() = 'authenticated'::text));

-- Policies for 'fretistas'
CREATE POLICY "Admins podem gerenciar todos os fretistas." ON public.fretistas FOR ALL USING (((SELECT tipo FROM public.usuarios WHERE (usuarios.id = auth.uid())) = 'administrador'::text));
CREATE POLICY "Usuários autenticados podem ler fretistas." ON public.fretistas FOR SELECT USING ((auth.role() = 'authenticated'::text));

-- Policies for 'notas_fiscais'
CREATE POLICY "Admins e colaboradores podem gerenciar todas as notas." ON public.notas_fiscais FOR ALL USING (((SELECT tipo FROM public.usuarios WHERE (usuarios.id = auth.uid())) = ANY (ARRAY['administrador'::text, 'colaborador'::text])));
CREATE POLICY "Gerência pode ler todas as notas." ON public.notas_fiscais FOR SELECT USING (((SELECT tipo FROM public.usuarios WHERE (usuarios.id = auth.uid())) = 'gerencia'::text));
CREATE POLICY "Fretistas podem ver suas notas." ON public.notas_fiscais FOR SELECT USING (
  ((SELECT tipo FROM public.usuarios WHERE (usuarios.id = auth.uid())) = 'fretista'::text) 
  AND 
  (placa_veiculo = (SELECT fretista_placa FROM public.usuarios WHERE (id = auth.uid())))
);

-- Policies for 'historico_acessos'
CREATE POLICY "Admins podem ver todo o histórico." ON public.historico_acessos FOR SELECT USING (((SELECT tipo FROM public.usuarios WHERE (usuarios.id = auth.uid())) = 'administrador'::text));
CREATE POLICY "Usuários podem inserir seu próprio histórico." ON public.historico_acessos FOR INSERT WITH CHECK ((auth.uid() = usuario_id));
